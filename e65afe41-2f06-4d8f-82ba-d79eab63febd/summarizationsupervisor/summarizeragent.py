...
class SummarizerAgentAgentExecuter(AgentExecutor):

    def __init__(self):
        self.agent_config = {"name": "summarizeragent", "prompt": "You are an expert agent named SummarizerAgent. Your goal is: Generate a concise (≤100 words), coherent summary of any input text, ensuring the summary preserves the main points and original context.. Your expertise includes: You are an advanced text summarization specialist, extensively trained in natural language processing and semantic understanding. You excel at parsing complex documents, extracting salient details, and synthesizing information into clear, non-redundant summaries. Your domain knowledge spans news, scientific literature, business documents, and more, enabling you to maintain context and relevance in any summary. To complete your tasks, you can use following tools: summarize_text", "tools": []}
    ...
    async def execute(
        ...
        try:
            ...
            raise ValueError("Error executing the SummarizerAgent Agent.")
...
def create_agent_card(host: str, port: int | str):
    ...
    AgentSkill(
        ...
        description='Generates a concise (≤100 words), coherent summary of any input text, preserving main points and original context.',
        tags=['text summarization', 'text condensation', 'context preservation'],
        examples=['Summarize the following article in under 100 words.', 'Give a brief summary of this text.'],
        input_modes=["text"],
        output_modes=["text"]
    )
    ]
    ...
    agent_card = AgentCard(
        name='Summary Agent',
        description='Generates concise, coherent summaries (≤100 words) of input texts, ensuring main points and context are preserved.',
        url=f'http://{{host}}:{{port}}/summary_agent/',
        version='1.0.0',
...
def build_component_identifier_app(configured_card: AgentCard):
    executor = SummarizerAgentAgentExecuter()
...
if __name__ == "__main__":
    ...
    reminderstandardcollectionagent_app = Starlette(
        routes = [
            ...
            Mount("/summary_agent/", app_with_card),                      
        ],
        middleware=[
